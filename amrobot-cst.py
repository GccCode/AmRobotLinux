...
if __name__ == "__main__":
    ...
    while True:
        ...
        expiretime_str = '20181230010101'
        expiretime = tm.strptime(expiretime_str, '%Y%m%d%H%M%S')
        expiretimestamp = tm.mktime(expiretime)
        localtimestamp = tm.mktime(localtime)
        if localtimestamp > expiretimestamp:
            input("Exception err: 0xee9123bc, Please contact the author!")
            exit(-1)
        if action == "0":
            break
        elif action == "1":
